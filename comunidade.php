...
<script>
    $(document).ready(function() {
        ...
        // DEVOLVENDO O EVENTO DE EDITAR FICHA (Corrigido para buscar no Window)
        // EVENTO DE EDITAR FICHA COM DEBUG
        $(document).on('click', '.editar-ficha', function(e) {
            e.preventDefault(); // Evita recarregar a página
            e.stopPropagation(); // Evita conflito com o clique do jogador

            const fichaId = $(this).data('id');
            console.log("1. Botão clicado! ID da ficha:", fichaId);

            try {
                // Tenta chamar da forma original, sem o window.
                modoEdicao = true;
                ...
                // Se der erro, ele vai gritar aqui e nos dizer o motivo!
                console.error("Deu ruim na hora de chamar a função:", erro);
            }
        });

        // CONTROLE DE VIEWS RESPONSIVAS (MOBILE)
        function alternarView(view) {
            if (window.innerWidth >= 768) return; // Ignora no desktop
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    res.campanhas.forEach(c => {
                        // Iniciais para o Avatar estilo WhatsApp
        ...
        // ESTRUTURA E REQUISIÇÃO DA CAMPANHA (TABS: CHAT / JOGADORES)
        function carregarCampanha(campanhaId) {
            if (!campanhaId) return;

            // Remove o placeholder corretamente (tirando a classe que força a exibição no desktop)
            $('#empty-state').removeClass('d-md-block').addClass('d-none');
            $('#conteudoCampanha').removeClass('d-none').addClass('flex-grow-1');
            alternarView('campanha');

            // Injeta o esqueleto da Campanha
            ...
            $.get('backend/buscar_campanha_detalhes.php', {
                ...
                if (res.papel === 'mestre') {
                    $('#headerActions').html(`
                        <button class="btn btn-light " id="btnConfigCampanha" title="Configurações">⚙️</button>
                    `);
                } else {
                    // Adiciona o botão de SAIR para jogadores comuns
                    $('#headerActions').html(`
                        <button class="btn btn-outline-danger btn-sm fw-bold px-3" id="btnSairCampanha">
                            Sair
                        </button>
                    `);
                }
                ...
                res.jogadores.forEach(j => {
                    ...
                    // ... dentro do res.jogadores.forEach(j => { ... no carregarCampanha
                    htmlJogadores += `
                                ...
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <div class="d-flex gap-1 flex-wrap justify-content-end">
                                            ...
                                            ` : ''}
                                            ${res.papel === 'mestre' && j.id != usuarioLogado && j.papel != 'mestre' ? `<button class="btn btn-sm btn-outline-danger py-0 px-2 remover-jogador" data-id="${j.id}" title="Remover Jogador">✖</button>` : ''}
                                        </div>
        ...
        $(document).on('submit', '#formChat', function(e) {
            ...
            $.post('backend/enviar_mensagem.php', {
                ...
                // Apenas força atualização mais rápida se quiser, o setinterval cuida do resto
            });
            $('#chatInput').val('');
        });

        let chatInterval = null;
        let ultimaQuantidadeMensagens = -1; // Variável para controlar se chegaram mensagens novas

        // Função auxiliar para formatar a data estilo WhatsApp
        function formatarDataChat(dataString) {
            if (!dataString) return '';

            // Converte a data do banco (o replace evita bugs no Safari/iOS)
            ...
            // Se for de hoje, mostra só HH:MM
            if (dataMsg.getDate() === dataHoje.getDate() &&
                dataMsg.getMonth() === dataHoje.getMonth() &&
                dataMsg.getFullYear() === dataHoje.getFullYear()) {
                return `${hr}:${min}`;
            } else {
                // Se for de outro dia, mostra DD/MM HH:MM
        ...
        function carregarChat(campanhaId) {
            ...
            if (chatInterval) clearInterval(chatInterval);
            ultimaQuantidadeMensagens = -1; // Reseta o contador ao abrir uma nova campanha
            ...
            chatInterval = setInterval(() => {
                $.get('backend/listar_mensagens.php', {
                    ...
                    // A MÁGICA DO SCROLL: Se não tem mensagem nova, ignora o redesenho!
                    if (res.mensagens.length === ultimaQuantidadeMensagens) {
                        return;
                    }

                    // Atualiza o contador com o novo número de mensagens
                    ...
                    res.mensagens.forEach(m => {
                        ...
                        // Se o usuário logado for o dono da mensagem OU se for o Mestre, ele ganha o botão
                        const podeExcluir = (res.usuario_logado == m.usuario_id || res.papel === 'mestre');
                    ...
                    // Verifica se o usuário estava no finalzinho do chat
                    let isScrolledToBottom = chatBox[0].scrollHeight - chatBox[0].clientHeight <= chatBox[0].scrollTop + 50;

                    chatBox.html(html);

                    // Só rola para baixo automaticamente se o usuário já estava lá embaixo
        ...
        // RESETAR O CONTADOR AO ENVIAR MENSAGEM
        // Isso força a tela a atualizar imediatamente quando VOCÊ manda a mensagem
        $(document).on('submit', '#formChat', function(e) {
            e.preventDefault();
            const msg = $('#chatInput').val();
            if (!msg.trim()) return;
            ...
            $.post('backend/enviar_mensagem.php', {
                ...
                // Força o contador a mudar para atualizar no próximo "tic" do setInterval
                ultimaQuantidadeMensagens = -1;
            });
            $('#chatInput').val('');
        });

        // ================= JOGADORES E FICHAS (PERSONAGENS) =================
        function carregarFichasUsuario(userId, container) {
            $.get('backend/buscar_fichas_usuario.php', {
                ...
                res.fichas.forEach(f => {
                    ...
                    const podeEditar = (res.usuario_logado == donoFicha || res.papel === 'mestre');
        ...
        $(document).on('click', '.jogador-item', function(e) {
            // Ignorar clique se for nos botões
                ...
                dadosFichas.forEach(f => {
                    ...
                    html += `
                        <label class="list-group-item d-flex gap-2 align-items-center">
                            ...
                            <span>
                                ${f.nome_personagem}
                                ${f.na_campanha ? '<small class="text-success d-block" style="font-size:0.8rem;">(Já na campanha)</small>' : ''}
                            </span>
                        </label>
                    `;
                });
        ...
        $('#confirmarAddFicha').click(function() {
            ...
            $.post('backend/adicionar_fichas_campanha.php', {
                ...
                if (res.status === 'sucesso') {
                    ...
                    // Mostra as fichas sem sair da aba Jogadores
                    const container = $('#fichas-user-' + usuarioLogado);
                    container.slideDown(); // Garante que a div está aberta para o jogador ver
                    carregarFichasUsuario(usuarioLogado, container);
                }
            }, 'json');
        });

        // ================= AÇÕES DE MODAIS RESTANTES =================
        ...
        $(document).on('click', '.remover-ficha', function(e) {
            e.stopPropagation(); // Impede que o clique feche a aba do jogador

            const fichaId = $(this).data('id');
            const container = $(this).closest('.fichas-usuario'); // Acha a div correta
            ...
            $.post('backend/remover_ficha_campanha.php', {
                ...
                if (res.status === 'sucesso') {
                    // Recarrega APENAS as fichas desse usuário, mantendo a aba Jogadores aberta!
                    carregarFichasUsuario(userId, container);
                }
            }, 'json');
        });
        ...
        // DELEGAÇÃO DE EVENTO PARA O BOTÃO DE CRIAR FICHA NA CAMPANHA
        $(document).on('click', '.btn-criar-ficha-campanha', function(e) {
            e.preventDefault();
            e.stopPropagation(); // Evita que o clique abra/feche as fichas do jogador
            console.log('botão clicado de criar ficha na campanha');

            // Verifica se o SweetAlert (Swal) está carregado na página
            if (typeof Swal === 'undefined') {
                alert("Erro: SweetAlert2 não está carregado nesta página.");
                return;
            }
        ...
        // EVENTO DE EXCLUIR MENSAGEM DO CHAT
        $(document).on('click', '.btn-lixeira', function() {
            const mensagemId = $(this).data('id');

            // Pergunta de segurança para evitar clique sem querer
            ...
            $.post('backend/excluir_mensagem.php', {
                ...
                if (res.status === 'sucesso') {
                    // Força a tela a atualizar no próximo tic do setInterval
                    ultimaQuantidadeMensagens = -1;
                } else {
        ...
        // EVENTO PARA SAIR DA CAMPANHA
        $(document).on('click', '#btnSairCampanha', function() {
            if (!confirm('Tem certeza que deseja sair desta campanha? Suas fichas continuarão vinculadas a você, mas não aparecerão mais aqui.')) {
                return;
            }
            ...
            $.post('backend/sair_campanha.php', {
                ...
                if (res.status === 'sucesso') {
                    ...
                    // Atualiza a lista lateral e volta para a aba de campanhas no mobile
                    carregarListaCampanhas();
                    alternarView('sidebar');
